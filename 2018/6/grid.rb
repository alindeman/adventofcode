...
class Grid
  ...
  def area_containing_safe_distances(safe_distance = 10_000)
    bounds[0].x.upto(bounds[1].x).sum { |x|
      bounds[0].y.upto(bounds[1].y).sum { |y|
        if @points.map { |p| p.manhattan_distance_from(Point.new(x, y)) }.sum < safe_distance
          1
        else
          0
        end
